...
def create_random_vacancy_based_on_distance(poscar_file, farthest=False):
    # Baca file POSCAR
    ...
    # Temukan indeks atom terdekat atau terjauh
    atom_index = find_nearest_or_farthest_atom_indices(structure, center_point, farthest=farthest)
    
    # Buat struktur baru dengan vacancy
    modified_structure = structure.copy()
    modified_structure.remove_sites([atom_index])
    
    # Hitung presentase vacancy
    total_atoms = len(structure)  # Menggunakan len(structure) sebagai jumlah total atom
    vacancy_count = 5  # Jumlah vacancy dalam contoh ini
    ...
    with open(output_filename, "w") as f:
        ...
        f.write(modified_structure.to(fmt="poscar"))  # Menyimpan struktur atom setelah vacancy dihilangkan
    ...
    with open(original_output_filename, "w") as f:
        ...
        f.write(structure.to(fmt="poscar"))  # Menyimpan struktur atom asli sebelum vacancy dihilangkan
...
create_random_vacancy_based_on_distance(poscar_file)

# Contoh penggunaan untuk vacancy berdasarkan jarak terjauh
create_random_vacancy_based_on_distance(poscar_file, farthest=True)
